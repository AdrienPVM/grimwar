...
### Dette regex damage → mapping canonique (plan 12)
Le cast flow extrait une formule `NdX[+/-Y]` depuis la description FR via regex `/(\d+d\d+(?:[+-]\d+)?)/i` (cf. `spell-detail-modal.tsx:25`). Confirmé silencieux non-bloquant : pas de match → `null` → toast fallback (pas de dégâts affichés, pas de throw). `rollDamage` ne throw pas non plus parce que la regex de l'extraction est plus permissive que celle de validation. **Mais** : risque de rater les dés multiples (« 8d6 » OK / « 1d8 acide + 1d4 feu » → ne prend que le premier), les dégâts à niveau supérieur (« +1d6 par niveau au-dessus du 1er » → ignoré), et les écritures en lettres. **Plan 12 doit remplacer par un mapping canonique `spellId → damage formula(s)`** extrait pendant `build-public-content` à partir du SRD. L'API de `SpellDetailModal` reste identique côté composant.
