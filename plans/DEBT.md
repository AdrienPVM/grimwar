...
- **Statut** : **purge partielle (3/7 livrés)**. Investigation 2026-05-28 (marathon nuit 4) : les 4 placeholders restants dans `tests/e2e/deferred-debt.spec.ts` (Sheet foundation, Essence, Avoir, Dice digital) sont **techniquement déblocables** — le fixture `seedCharacter` (Admin SDK contre l'émulateur) est livré au complément 13.5, les 5 modes sheet (`combat`/`magie`/`essence`/`avoir`/`ame`) sont **fonctionnellement présents** (cf. `src/features/sheet/modes/`) malgré le plan 20 formel non coché. **Coût estimé** : ~2-3h focus pour les 4 specs en série + émulateur Java tournant en parallèle. **Décision (marathon nuit 4)** : ces 4 specs restent ownerisées plan 20.5 et **ne sont PAS livrées par un chantier opportuniste** — le scope est cohérent avec un plan dédié (anti-régression S1 close-out), pas un patch nocturne. Aucune perte de couverture critique observée : la logique pure de ces 4 features est déjà couverte par unit tests (`hp-state.test.ts`, `roll-with-flags.test.ts`, `inventory-rules.test.ts`, `use-dice.test.ts`). Le filet golden-path est en place (32 specs e2e actives, dont 28 fonctionnent contre l'émulateur — couvrant smoke / wizard / library-render / combat / magie / dice-physical / ancestry × 5 / class × 3 / sheet-defense / sheet-summoned-statblock / sheet-spell-damage / map-phase2 / responsive / wizard-modal etc.).
...
- **Sous-dettes après séquence D13a-e** :
  ...
  - **Followups post-marathon** (mini-plans dédiés) :
    ...
    - ~~`D13e-followup-grant` (volet chooser)~~ → **RÉSOLU 2026-05-26
      ...
      `.pactTomeRituals`.
    - ~~`D13e-followup-grant-display`~~ → **RÉSOLU 2026-05-28** au commit
      `37a6aa4` (PR #41, squash `9b45383`). Les 3 sorts mineurs +
      2 rituels L1 grantés par le Pacte du grimoire sont rendus en mode
      Magie avec chip dédié « Pacte du grimoire ». Per SRD FR 5.2.1
      p. 142, ils sont injectés dans `knownSet` + `preparedSet` de
      `SpellList` (visuel doré « préparé »). Nouveau module
      `pact-tome-source-label.ts` + map `pactTomeSourceLabels` calculée
      dans `MagieMode` + nouvelle prop sur `SpellList` et
      `SpellDetailModal`. Voir `## Résolu` ci-dessous.
...
## D21 — CI GitHub Actions ne déclenche plus sur les PR (probable quota free tier épuisé) — **RÉSOLU 2026-05-28**

- **Owner** : Adrien (investigation manuelle Settings → Billing → Actions usage).
- **Statut** : **RÉSOLU 2026-05-28** — contournement définitif : le repo `AdrienPVM/grimwar` est passé public, ce qui ouvre **minutes Actions illimitées** sur tier free (GitHub-hosted runners gratuits sur les repos publics). Plus aucun blocage CI quota observé depuis (D phase 2 = 6 PRs successives avec CI 5/5 verte sur chacune, plus D13e-followup-grant-display PR #41 verte). Voir l'historique d'incident ci-dessous pour la trace de l'investigation initiale.
- **Statut initial** : **OUVERT 2026-05-26** — découvert en clôture du PR #19 (CHANTIER 1 marathon nuit 3). Le workflow `.github/workflows/ci.yml` ne se déclenche PAS sur aucun push ni `pull_request` ouverture/reopen sur la branche `chore/13.14-desktop-prototype-combat-magie` (3 pushes + 1 close+reopen = 0 run créé). Le dernier run réussi sur le repo date du PR #18 (chore/map-proto-skeleton, 2026-05-26T09:04 UTC, 11m41s, success).
- **Diagnostic réalisé** :
...
- **Drifts au tracer C.4** :
  - `munition-1-2-ou-3` : `name.fr "Munition +1, +2 ou +3"` → SRD FR officiel **`"Projectile +1, +2 ou +3"`** (slug stable).
  - `arme-vigilante` : **NOUVELLE ENTRÉE AJOUTÉE** (Weapon of Warning — SRD officiel, mais absent du bundle baseline AideDD).
- **Drifts au tracer C.5** :
...
- **Owner** : `plans/D-map-phase2-ui.md`.
- **Statut** : **RÉSOLUE 2026-05-28** — 6/6 tracer-bullets D.1..D.6 livrés (PR #35 `f082ff7`, PR #36 `f2381ff`, PR #37 `ffb6031`, PR #38 `7c8e92e`, PR #39 `5fe47fd`, PR #40 `05d63b2`). Voir détails en section « Résolu ».
...
- **D23 — `potion-de-souffle-enflamme` taggué `basic-rules` alors qu'il n'est PAS dans le SRD CC v5.2.1** — **résolu 2026-05-28** au commit `2dacd5a` (PR #45, squash `cc17508`). Option (b) retenue par Adrien : re-tag `source: "aidedd-homebrew"` (l'enum acceptait déjà cette valeur, anticipée pour ce cas). Vérification SRD verbatim : `grep -i "fire breath\|souffle enflammé\|potion of fire"` sur `content-sources/extracted/raw/{SRD,FR_SRD}_CC_v5.2.1.txt` — « Fire Breath » apparaît uniquement comme capacité de stat-block de dragons (Recharge 5-6), jamais comme potion. L'entrée est un legacy AideDD grandfathered. Option (a) « retirer » écartée — destructif sur inventaires existants éventuels, inutile dès lors que le tag est précis. Changements : (1) `public/data/magic-items.json:3124` — `source: "basic-rules"` → `"aidedd-homebrew"` (1 ligne) ; (2) `public/data/index.json` — `contentHash` régénéré (`7c236d03bea6…`) via `pnpm tsx scripts/update-content-index.ts` ; (3) `tests/d23-potion-souffle-enflamme-source.test.ts` (NEW) — garde-fou anti-régression qui assert que la potion reste tagguée `aidedd-homebrew` (jamais `srd-5.2.1` ni `basic-rules`). Rouge-avant-vert prouvé manuellement (le test fail sur l'état pré-fix avec « expected 'basic-rules' to be 'aidedd-homebrew' »). Quadruple gate Node 22 verte : typecheck + lint + 1624 unit + 69 e2e. **Cleanup magic-items partiel** : D24 (doublet amulette-de-sante / bonne-sante) reste ouvert, en attente d'arbitrage Adrien.
- **D21 — CI GitHub Actions ne déclenche plus sur les PR (quota free tier épuisé)** — **résolu 2026-05-28** par bascule du repo `AdrienPVM/grimwar` en **public**. Sur les repos publics, GitHub fournit **des minutes Actions illimitées sur les runners GitHub-hosted** (tous tiers, y compris free) — le quota n'existe tout simplement plus. Contournement définitif, pas patch transitoire. **Validation empirique** : depuis la bascule, le marathon nuit 4 a livré 6 PRs successives (D phase 2 — PRs #35 → #40) avec **CI 5/5 verte sur chacune** (4 required + protected-paths-guard post-merge sur main), plus **D13e-followup-grant-display PR #41** verte (CI run `26573650724`). Aucun blocage CI quota observé. **Effet collatéral positif** : le projet est désormais publiquement visible — pas un retrait, l'intention de commercialisation reste valide (cf. CLAUDE.md « Commercial intent »), mais le code source ouvert facilite review/portfolio. **Documentation** : la section D21 historique est conservée plus haut pour la trace de l'investigation initiale (hypothèses, diagnostic, marche à suivre éventuelle si récurrence) — utile si le repo est repassé en privé un jour.
- **D13e-followup-grant-display — rendu des 5 sorts grantés par le Pacte du grimoire en mode Magie** — **résolu 2026-05-28** au commit `37a6aa4` (PR #41, squash `9b45383`). **Périmètre purement visuel** : la data est posée depuis `D13e-followup-grant` (CHANTIER B nuit 3) — `classes[i].pactTomeCantrips` (3 cantrips) + `.pactTomeRituals` (2 rituels L1). Avant cette dette, ces 5 sorts étaient persistés mais invisibles côté fiche. Désormais ils apparaissent dans la `SpellList` du mode Magie avec un chip dédié « Pacte du grimoire » (amethyst, terminologie WotC FR officielle = nom de l'invocation), et tap → modale détail avec le même chip dans l'en-tête. **Décision SRD** : per `FR_SRD_CC_v5.2.1.pdf` page 142, « tant que le grimoire est sur vous, les sorts choisis sont **préparés et fonctionnent comme des sorts d'Occultiste** » → injectés dans `knownSet` + `preparedSet` (visuel doré « préparé », pas amethyst « non-préparé »). Les rituels L1 portent en plus leur badge `Rituel` (teal) hérité du flag `spell.ritual` du bundle. **Implémentation** : (1) module pur `src/features/sheet/modes/magie/pact-tome-source-label.ts` + 7 tests cat. 4/5/6 ; (2) extension `MagieMode` qui calcule `pactTomeSourceLabels` via `useMemo` et le passe à `SpellList` + `SpellDetailModal` ; (3) extension `SpellList` (nouvelle prop `pactTomeSourceLabels: ReadonlyMap<string, string>`, injection dans `knownSet`/`preparedSet`/`sourceMap.fromPactTome`, chip amethyst dédié) ; (4) extension `SpellDetailModal` (prop `pactTomeSource: { label } | null`, chip dans l'en-tête) ; (5) extension i18n FR/EN (`sheet.magie.pactTome.sourceLabel`). **Tests** : 5 tests SpellList « D13e — Pacte du grimoire » (rouge-avant-vert vérifié manuellement : 4/5 tests fail sans le patch `pactTomeSourceLabels → knownSet`) + 7 tests helper + 2 tests MagieMode (intégration warlock E2E in-vitro) + 1 nouveau test e2e Playwright (`class-warlock-invocation.spec.ts` D13e-followup-grant-display : seed Velinor warlock L1 avec 5 sorts grantés → assertion des 5 noms visibles dans le mode Magie + chip + modale). Quadruple gate Node 22 verte : 1622 unit + 15 skipped + 69 e2e (4 skipped) + lint + typecheck. Aucune régression sur les 23 tests pré-existants de la SpellList / SpellDetailModal / MagieMode (props ancestrySource intactes). **Décision de scope (autonomie totale)** : pas d'intégration mécanique différée — le bouton « Lancer » fonctionne déjà via le flux existant warlock (ability = Cha, pas de slot pour cantrips, pour les rituels c'est le même comportement qu'un sort L1 connu par la classe). Pas de nouveau type de slot, pas de nouvelle stat. **Pas de changement de schéma `Character`** — les champs `pactTomeCantrips`/`pactTomeRituals` existaient déjà depuis CHANTIER B nuit 3. **Surface impactée** : `src/features/sheet/modes/magie-mode.tsx` (calcul + passage props), `src/features/sheet/modes/magie/spell-list.tsx` (prop + knownSet + chip), `src/features/sheet/modes/magie/spell-detail-modal.tsx` (prop + chip), `src/features/sheet/modes/magie/pact-tome-source-label.ts` (NEW), `src/shared/lib/i18n.ts` (clé FR + EN), `tests/e2e/seed-character.ts` (étend `SeedClassSubChoices` avec `pactTomeCantrips`/`pactTomeRituals`/`pactBladeWeapon` + enrichit le seed `warlockL1PactOfTheTome` avec 3 cantrips + 2 rituels SRD). Owner = `plans/D13e-followup-grant-display.md`.
- **D25 — Mode carte phase 2 : UI complet + services Firestore live** — **résolu 2026-05-28** (marathon nuit 4). 6 tracer-bullets D.1..D.6 livrés en séquence, chacun mergé individuellement après quadruple gate Node 22 verte + CI 5/5 sur main : (1) **D.1** PR #35 `f082ff7` — `src/shared/lib/services/maps.ts` + 12 unit tests mockés Firestore (createMap/updateMap/deleteMap/createToken/updateToken/deleteToken + add/remove fog/light/aoe). (2) **D.2** PR #36 `f2381ff` — `use-maps-list.ts` listener `onSnapshot campaigns/{cid}/maps/*` + 6 unit tests. (3) **D.3** PR #37 `ffb6031` — `MapsCloudScreen` (route `/map-proto/cloud/:cid`) + helper `ensureCampaignExists` (campaigns.ts) + 5 tests campaigns + 13 tests screen. Convention conservative : `cid` arrive par URL parce que la feature `campaigns/` (S2) n'est pas livrée. (4) **D.4** PR #38 `7c8e92e` — `MapLiveScreen` (route `/map-proto/cloud/:cid/maps/:mid`) avec drag-and-drop token → `updateToken` au pointerUp + 10 unit tests avec stubs JSDOM SVG. (5) **D.5** PR #39 `5fe47fd` — Persistance fog/lights/AoE (3 sections de boutons « Reveal au centre / Mask au centre / Effacer fog », « Torche au centre / Effacer lumières », « Sphère 20 ft au centre / Effacer AoE ») + 10 unit tests. (6) **D.6** PR #40 `05d63b2` — Spec e2e `tests/e2e/map-phase2-uat.spec.ts` parcours MJ end-to-end + 3 fixes découverts en jouant la spec : **fix rules** `firestore.rules:235,253` (DM sans membership ne pouvait pas lire ses cartes — read passe à `isDMOf || isMemberOf`, en cohérence avec `campaigns/{cid}` ligne 156), **fix permission-flow** `ensureCampaignExists` (fall-through `setDoc` sur permission-denied au `getDoc`), **fix race condition** `MapsCloudScreen` (gate `useMapsList` sur `ensureDone`). Nouveau test rouge-avant-vert dans `tests/firestore-rules.test.ts` (« DM sans membership peut lire ses propres maps »). Quadruple gate Node 22 finale verte : 1339 unit + 15 rules + 68 e2e. **À déployer** : `pnpm firebase:deploy:rules` avant prochain plan consommateur en prod (la route `/map-proto/cloud/:cid` n'est pas listée au menu et le projet n'est pas en prod publique — pas bloquant immédiat). **Hors scope phase 2** : Firebase Storage (upload image), self-write par PJ sur token lié (`linkedCharacterId == ma fiche`), édition complète `MapMeta`, suppression sélective d'un polygon/lumière/AoE. Différés à `map-phase3` ou follow-ups dédiés.
