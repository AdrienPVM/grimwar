...
- **Statut** : ouverte. Tracée 2026-05-25 à la clôture du plan D1 (43 sorts couverts / ~87 sorts SRD à dégâts identifiés). **Re-inventaire 2026-05-25** : `node` direct contre `public/data/spells.json` recense 124 sorts SRD à formule `\d+d\d+` détectable ; après scrub manuel des faux positifs (heals + bonus aux jets + durées + table rolls), il reste **~55 sorts à dégâts réels** à modéliser et **~20 sorts à exclure explicitement** (heals, bonus, durées, table rolls). Voir `plans/D1a-spell-damage-longtail.md > Inventaire initial` pour la liste exhaustive par batch.
