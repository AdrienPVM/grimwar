...
- **Statut** : **EN COURS 2026-05-28** — D.1..D.5 livrés (PR #35 `f082ff7`, PR #36 `f2381ff`, PR #37 `ffb6031`, PR #38 `7c8e92e`, PR #39 `5fe47fd`), reste D.6 (e2e parcours MJ).
...
- **Tracer-bullets** :
  ...
  - **D.3 — UI MJ création de carte (MapsCloudScreen)** ✅ mergé PR #37 `ffb6031` — `src/features/map-proto/maps-cloud-screen.tsx` (liste temps réel + form création slug+name + suppression inline, surface erreurs ensure/list/form) + route `/map-proto/cloud/:cid` + helper `src/shared/lib/services/campaigns.ts` `ensureCampaignExists` (no-op si existe, sinon setDoc stub conforme rules `dmUserId == auth.uid`). 5 tests campaigns + 13 tests maps-cloud-screen. Convention conservative documentée : le `cid` arrive par URL parce que la feature `campaigns/` (S2) n'est pas livrée — la route reste PROTOTYPE jusqu'au selector S2.
  - **D.4 — Vue live d'une carte (MapLiveScreen)** ✅ mergé PR #38 `7c8e92e` — `src/features/map-proto/map-live-screen.tsx` (consomme `useMap(cid, mid)` phase 1, rend SVG grille + bg + tokens, drag-and-drop → `updateToken` au pointerUp avec override local pendant le drag, surface d'erreur de write). Route `/map-proto/cloud/:cid/maps/:mid`. 10 unit tests Vitest avec stubs JSDOM (`createSVGPoint`/`getScreenCTM`/`setPointerCapture`). Le proto local `/map-proto` (localStorage) reste inchangé — les deux routes coexistent. Hors scope D.4 : création/suppression de tokens depuis cette vue, fog/light/AoE persistence (D.5), édition `MapMeta`.
  - **D.5 — Persistance fog of war + lumière + AoE depuis UI** ✅ mergé PR #39 `5fe47fd` — `map-live-screen.tsx` étendu avec 3 sections de boutons dans le header : Fog (Reveal au centre / Mask au centre / Effacer fog), Lumières (Torche au centre / Effacer lumières), AoE (Sphère 20 ft au centre / Effacer AoE). Tous les writes via service maps.ts D.1 (`addFogPolygon` / `addLightSource` / `addAoeTemplate` / `updateMap` pour les clears). IDs slug-safe `manual-{kind}-{8chars}`. +10 unit tests. Hors scope D.5 : rendu visuel des fog/lights/AoE sur le SVG (D.6 ou plus tard), suppression sélective.
