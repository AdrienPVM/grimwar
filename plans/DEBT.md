...
- **Sous-dettes après séquence D13a-e** :
  ...
  - **Followups post-marathon** (mini-plans dédiés) : `D13c-followup-attacks-list` (intégration arme de pacte côté Combat mode avec attack roll Cha) ; `D13c/d/e-followup-chooser` (3 choosers wizard L1 : arme bonded / forme familier / 5 sorts grimoire) ; `D13d-followup-statblocks` (4 stat blocks familiers à ajouter à `summoned-creatures.json`) ; `D13d-followup-summary` (gap bundle 4 formes vs SRD 7) ; `D13e-followup-grant` (grant `knownSpells['warlock-tome']`).
