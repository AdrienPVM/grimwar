...
- **Sous-dettes ouvertes après D13a/b** :
  - **D13b** — **LIVRÉ 2026-05-26** (CHANTIER 2 marathon nuit 2). Voir `## Résolu` ci-dessous. `eldritch-mind` → `kind: 'passive-concentration-advantage'` ajouté au discriminated union, registre peuplé, helper `hasConcentrationAdvantage(classes)` livré (parallèle à `computeInvocationAcBonus`). Rendu modal `<InvocationEffectCard>` câblé pour le nouveau kind. Consommation côté moteur Concentration save différée à D24 (encounters) — l'info est exposée au sheet mais pas encore appliquée aux jets de dés.
