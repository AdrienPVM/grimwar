...
- **Sous-dettes ouvertes après D13a/b/c/d** :
  ...
  - **D13d** — **LIVRÉ 2026-05-26** (CHANTIER 4 marathon nuit 2). Voir `## Résolu` ci-dessous. `pact-of-the-chain` → `kind: 'feature-pact-chain-familiar'` ajouté (2e feature active), registre peuplé avec Find Familiar gratuit + 4 formes spéciales (Imp/Pseudodragon/Quasit/Sprite) + magic-action + noSlotRequired. Helper `hasPactOfTheChain(classes)` exposé. Modale rend 3 lignes structurées + caveat « différé ». **Pas de statblocks** des 4 familiers ajoutés au bundle `summoned-creatures.json` (différé à D13d-followup-statblocks). **Pas de chooser de forme** au wizard (différé à D13d-followup-chooser). **Bundle summary FR/EN** (`pact-of-the-chain.summary`) liste 4 formes vs SRD 5.2.1 réel 7 (Imp/Pseudodragon/Quasit/Skeleton/Sphinx of Wonder/Sprite/Venomous Snake) — gap tracé en D13d-followup-summary.
