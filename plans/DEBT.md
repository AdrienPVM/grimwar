...
- **Statut** : ouverte. Tracée 2026-05-19 à la livraison du plan 13.8b ; **surface élargie 2026-05-23/24** par la résolution D18 (les sorts de traits communs sont désormais **présents** dans `knownSpells.ancestry` mais leur cast reste désactivé — la surface castable s'agrandit donc).
...
## D1a — `spell.damage[]` long-tail (~44 sorts SRD non couverts par D1)

- **Owner** : à créer comme mini-plan dédié post-D1 (ou greffé au plan 19 Bibliothèque qui montre les filter chips par type de dégâts — la valeur utilisateur croît avec la couverture).
- **Statut** : ouverte. Tracée 2026-05-25 à la clôture du plan D1 (43 sorts couverts / ~87 sorts SRD à dégâts identifiés par `jq '.[] | select(.description.en | test("\\dd[0-9]+ [A-Z][a-z]+ damage"))'`).
- **Liste indicative non-exhaustive** : faveur-divine, chatiment-divin (paladin smites), epine-mentale, croissance-d-epines, lame-de-feu, metal-brulant, rayon-affaiblissant, rayon-de-lune, esprits-gardiens, fusion-dans-la-pierre, invocation-d-animaux, malediction, mur-de-vent, assassin-imaginaire, bouclier-de-feu, chien-de-garde, controle-de-l-eau, invocation-d-etres-sylvestres, porte-dimensionnelle, tentacules-noirs, brume-mortelle, colonne-de-flamme, contact-avec-les-plans, contagion, quete, songe, barriere-de-lames, cercle-de-mort, contamination, mur-d-epines, mur-de-glace, sphere-glacee, embruns-prismatiques, invocation-de-celeste, symbole, teleportation, eclat-du-soleil, tremblement-de-terre, tsunami, ennemi-subconscient, mot-de-pouvoir-mortel, mur-prismatique, souhait.
- **Surface impactée** : `scripts/data/srd-spell-damage.ts` (add-by-slug) + tests cat. 4 par batch dans `tests/srd-spell-damage.test.ts`. Pattern documenté dans `plans/D1-spell-damage.md > Notes for next plan`.
- **Non-bloquant** : la regex stopgap `extractDamageFormula` reste en fallback pour ces sorts ; comportement utilisateur dégradé mais fonctionnel (chip dégâts absent dans la liste, modale rend la prose seule, autoroll de dégâts fonctionne via la regex).
- **Critère de complétion** :
  1. 100 % des sorts SRD identifiés ont une entrée `damage[]` canonique.
  2. Retrait de la regex `extractDamageFormula` + `DAMAGE_RE` (D1b — dépend de D1a).
  3. Test garde-fou : tout sort SRD avec « X[d]Y damage » dans la description EN doit avoir une entrée `damage[]` (intégrité référentielle inverse).
  4. Cette entrée bascule en `## Résolu` avec le hash du commit.
