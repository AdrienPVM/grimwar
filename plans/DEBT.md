...
- **Sous-dettes ouvertes après D13a/b/c** :
  - **D13b** — **LIVRÉ 2026-05-26** (CHANTIER 2 marathon nuit 2). Voir `## Résolu` ci-dessous.
  - **D13c** — **LIVRÉ 2026-05-26** (CHANTIER 3 marathon nuit 2). Voir `## Résolu` ci-dessous. `pact-of-the-blade` → `kind: 'feature-pact-weapon'` ajouté (1ère feature « active » du discriminated union, distincte des passifs), registre peuplé avec les 4 paramètres mécaniques (Cha + simple/martial melee + 4 types de dégâts + bonus-action). Helper `hasPactOfTheBlade(classes)` exposé. Modale rend 4 lignes structurées + caveat « différé ». **Pas de wizard chooser** pour pré-bonder une arme ni intégration `attacks-list` Combat mode — différés à un mini-plan dédié post-D13c (convention SRD : choix au moment de l'invocation in-game, annoncé au MJ).
