...
| **2D.5** | ✅ PR #89 (ce commit) | Matrix pins multi-class (3 tests : Fighter+Wizard slots unifiés, Paladin add Bard bloqué CHA, borne 4 classes) + doc clôture 2D. **e2e + UAT navigateur** restent à valider hors-CI (Java/JRE requis pour émulateur). | `tests/wizard-matrix/level-up-matrix.test.ts`, `plans/2D-MULTICLASS-AUDIT.md` | PR squash |
