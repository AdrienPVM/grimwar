...
- **Step 4 (recent events feed) — LIVRÉ INTÉGRALEMENT via JALON 22.3 + 22.4**, dans `campaign-detail-screen` (la vraie surface MJ d'une campagne réelle, cf. 4A.3), PAS dans la route `/campaign/:id/dm` décrite ici. Le `/dm` actuel est le **prototype 4A pré-V1** (opère sur mock-uid, cf. MVP-V1-DECISIONS-PRISES.md du 2026-06-04).
  - **22.3** : le feed d'activité MJ temps-réel (`CampaignEventFeed` + `useCampaignEvents`, rule `events` élargie `isMemberOf || isDMOf`, query contrainte par visibilité, filtre `canViewEvent`).
  - **22.4** (le « Tap an event for details. Filter by player » du step) :
    - **Détail au tap** — chaque ligne est un bouton qui ouvre `EventDetailModal` (réutilise la primitive `DetailModal` partagée). La modale montre l'enveloppe (type, **horodatage complet** via `formatEventDateTime`, **acteur résolu par NOM**, cible) + des lignes de détail **FR-étiquetées** dérivées du payload (`eventDetailRows` : avant/après/variation signée/cause, intitulé/dés/modificateur/total/crit, niveau/emplacement/composantes, quantité). **Aucune résolution de slug** (spellId/conditionId/itemRef n'apparaissent jamais — réservés au compilateur plan 25).
    - **Filtre par joueur** — chips « Tous » + un par personnage lié, **libellé = nom du personnage** résolu par `useLinkedCharacterNames` (lecture cross-owner **one-shot `getDoc`**, rule A2 ; fallback UID tronqué). Sélectionner un joueur ne montre que les événements dont `actorCharacterId` correspond (le jet secret du meneur, acteur null, disparaît). Filtre obsolète (joueur délié en cours de session) ignoré au lieu d'afficher un vide trompeur.
    - Zéro nouvelle rule, **zéro schéma, zéro index, zéro deploy** : pur consommateur de l'existant (rule A2 + query 22.3). Tests : `event-line.test.ts` (+`eventDetailRows`/`formatEventDateTime`), `campaign-event-feed.test.tsx` (filtre + modale), e2e `campaigns-dm-event-feed-detail.spec.ts` (filtre par nom + détail au tap contre les vraies rules).
...
## Notes for next plan
- Plan 22 implements the full event-logger — LIVRÉ (22.1/22.2 écriture, 22.3 lecteur, 22.4 détail+filtre). Le feed d'activité MJ est en ligne dans `campaign-detail-screen`, **step 4 du plan 21 désormais clos**.
- `useLinkedCharacterNames` (22.4) résout `characterId → nom` en lecture cross-owner one-shot (getDoc, rule A2). Réutilisable par tout futur lecteur MJ qui doit étiqueter un acteur/joueur par nom (ex. compilateur plan 25, panneau sessions plan 23). Choix one-shot assumé : un renommage joueur ne se reflète qu'au prochain montage — acceptable, à passer en live si un besoin temps-réel sur les noms émerge.
- `eventDetailRows` (event-line.ts) est l'expansion structurée de `summarizeEvent` : même périmètre de `kind`, mêmes garde-fous (jamais de slug). Le compilateur narratif plan 25 reste la couche qui résout les slugs en noms FR ; 22.4 ne l'anticipe pas.
