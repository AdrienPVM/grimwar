...
> - **UI consommatrice (JALON 4A.3)** : le MJ ouvre la fiche d'un joueur lié **en lecture seule** via `/campaigns/:cid/members/:memberUid/sheet` (affordance « Voir la fiche » du roster). La fiche est rendue avec `PermissionProvider value={{ canEdit: false, isDM: true }}` → tous les contrôles d'édition (HP ±, montée de niveau, équipement, lancement de sorts…) sont désactivés, et le FAB d'historique des jets est masqué (la sous-collection de jets vit hors périmètre de la rule A2). ⚠️ **Cette UI rend le deploy de `firestore.rules` (A2) obligatoire avant toute mise en prod** — sans deploy, le MJ obtient `permission-denied` à l'ouverture.
> - **READ co-membre** (« co-member read-only ») : **pas encore ouvert** — A2 est MJ-only.
> - **WRITE MJ** (DM omni-edit) : **pas encore câblé** — la fiche reste owner-write only. L'autorité d'écriture MJ + les restrictions field-level (`personality.*`, `name`, `homeCampaignId` owner-locked) arrivent à un jalon dédié (Cloud Function Admin SDK).
