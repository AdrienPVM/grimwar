...
- `docs/prototype/grimwar.html` (aesthetic illuminated-manuscript)
- `docs/plans/DEBT.md > D3` (cause-racine de cet abandon)
- `docs/plans/DEBT.md > D4` (dette `featAtLevel1` portée par le plan 14)
...
> Don au niveau 1 (`featAtLevel1`) : **hors scope** S1 du wizard. Dette tracée en `docs/plans/DEBT.md > D4`, owner = plan 14. Le wizard de création devra à terme lire `activeCampaign.settings.variants.featAtLevel1` et insérer une étape Don conditionnelle. Validé Adrien.
...
- [ ] C.2.g Aurora + flourishes du prototype préservés. Vérification visuelle vs `docs/prototype/grimwar.html` à chaque étape.
...
- [x] H.1 Ce fichier (`docs/plans/05-character-creation-wizard.md`) écrit en remplacement de `docs/plans/05-manual-character.md`.
- [x] H.2 `docs/plans/05-manual-character.md` supprimé.
- [x] H.3 `docs/plans/17-wizard-creation.md` réduit à un stub de redirection vers le plan 05.
- [x] H.4 `docs/plans/00-overview.md` mis à jour : ligne 05 renommée, ligne 17 marquée « ABANDONNÉ », total 43 → 42, swap consigné dans `## Swaps actés`.
- [x] H.5 `docs/ROADMAP.md` mis à jour : 43 → 42 plans, ligne 17 retirée du tableau S2.
- [x] H.6 `docs/plans/DEBT.md` : entrée D3 ouverte (wizard abandonné + 3 bugs structurels), entrée D4 ouverte (`featAtLevel1` portée par plan 14).
...
- [ ] D3 fermée dans `docs/plans/DEBT.md > ## Résolu` avec le hash du commit `feat(wizard):` et la mention que les 3 bugs sont structurellement absents.
...
- Plan 14 (campaigns model) hérite de la dette D4 (`featAtLevel1`) — voir `docs/plans/DEBT.md > D4` pour le contrat exact attendu côté wizard quand `activeCampaign.settings.variants.featAtLevel1 === true`.
