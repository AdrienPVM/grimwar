...
>
> **Mini-plan content+UI.** Résolvait `docs/plans/DEBT.md > D14`. Nouveau type de contenu `summoned-creatures` (schéma Zod + bundle `public/data/summoned-creatures.json` + extracteur SRD-only) + lien `spell.summonedCreatureIds[]` + rendu UI dans `SpellDetailModal`. Retire les 4 marqueurs `[Profil … suivi en dette D14.]` de `srd-spells.ts`. Branche dédiée + PR (paths protégés touchés). 4 commits + clôture docs.
...
- [x] 19. `docs/plans/DEBT.md > D14` : bascule en `## Résolu` avec hash du commit 3 ; mettre à jour la `## D14` historique avec la mention « résolue par plan D14 ».
...
- [x] `docs/plans/DEBT.md > D14` en `## Résolu` avec hash du commit.
