...
- `docs/prototype/grimwar.html` (open in a browser — this is the target aesthetic)
...
- [x] 1. Confirm the working directory is the project root (`grimwar/`). Verify `CLAUDE.md`, `docs/`, `docs/plans/`, `docs/prototype/grimwar.html` are present. _(All present, repo cloned at `C:\Users\adrie\Documents\GIT-PROJECTS\grimwar\`.)_
...
Plan 02 (Design system) builds on top: turns the inline placeholder styles into a real component library (GlassPanel, Card, Button, Aurora, Particles, Icon sprite, etc.). The SVG icon sprite content is in `docs/prototype/grimwar.html` — copy verbatim.
