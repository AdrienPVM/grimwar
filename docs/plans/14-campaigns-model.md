...
- [ ] 5. Route `/campaigns/new` → `<CampaignCreateScreen />`:
    ...
    - **Mode de dés par défaut** : segment Digital / Physique, écrit dans `settings.diceMode`, défaut `'digital'`. Tooltip FR : « Digital — l'app lance pour tout le monde. Physique — l'app indique quoi lancer, les joueurs saisissent les faces. » Chaque joueur peut décrocher via son toggle personnel (`followCampaignDiceMode: false`).
...
- [ ] 8. Campaign settings screen `/campaign/:id/settings`: edit name, description, language, allowHomebrew, startingLevel, status (active/paused/archived), **diceMode** (Digital / Physique). DM only.
...
- **Dice mode** : `campaigns/{id}.settings.diceMode` est exposé dans la création + settings. Le helper `effectiveDiceMode(user, campaign)` (plan 12) commencera à recevoir un `campaign` non-nul dès qu'une membership active existe ; jusqu'ici il recevait toujours `null`. Vérifier que `useActiveCampaign()` est branché au pivot `rollWithFlags` pour que les jets en campagne respectent le défaut de table.
