...
## Context
Read `docs/prototype/grimwar.html` (Magie section — magic circle), `docs/DATA-MODEL.md` (spell slots, prepared/known spells).
...
**Fallback ajouté post-UAT plan 09 (commit séparé)** : `hasPactProgression(character, classCatalog)` détecte une classe pact, et `MagicCircle` affiche un état vide dédié (« Magie de pacte — bientôt disponible. Les emplacements d'Occultiste suivent une progression indépendante… ») au lieu du générique « Aucun emplacement débloqué » qui était trompeur pour un Occultiste niv. 1. Les cantrips restent jouables. Le cas multi-class warlock + caster (slots non-pact visibles, slots pact absents) n'est PAS couvert par ce fallback — la circle montre uniquement les slots unifiés, c'est techniquement incomplet mais pas trompeur. À régler en plan 18.

### Dette regex damage → mapping canonique (consolidée)
Le cast flow extrait une formule `NdX[+/-Y]` depuis la description FR via regex `/(\d+d\d+(?:[+-]\d+)?)/i` (cf. `spell-detail-modal.tsx:25`). Confirmé silencieux non-bloquant : pas de match → `null` → toast fallback (pas de dégâts affichés, pas de throw). **Initialement targeté plan 12**, désormais consolidé avec la livraison `SpellDamageSchema` posée en plan 12 step 9 (forward-compat, jamais peuplé). **Dette unique tracée dans `docs/plans/DEBT.md > D1`** — owner = **plan 19 (Bibliothèque)**, premier surface utilisateur qui crée une valeur visible à partir de `spell.damage[]` structuré. La regex reste le **fallback** jusqu'à ce que le pipeline SRD peuple `spell.damage[]` ; à ce moment-là le moteur de dégâts de sort bascule sur la donnée canonique. Ne pas redécrire la dette dans les plans : pointer `DEBT.md > D1`.
