...
Ce fichier trace TOUTES les décisions UX/produit prises par défaut par GSD pendant le développement V1, parce qu'elles n'étaient pas explicitement couvertes par docs/plans/MVP-V1-SPEC.md.
...
### [JALON-4A.2] Picker « Mon personnage » — premier consommateur UI du link character→membership (2026-06-23)

**Contexte** : 4A.1 a livré le service `linkCharacterToMembership` + la rule de lecture MJ (A2 sur `homeCampaignId`), mais **aucun appelant UI** ne posait le `characterId` sur `members/{uid}`. Conséquence directe (gravée en 4A.1) : chaque fiche live ayant `homeCampaignId: null`, la rule A2 était **comportementalement identique au stub owner-only** — l'autorisation MJ ne pouvait jamais s'ouvrir faute de donnée. 4A.2 câble le consommateur : le joueur lie/délie sa fiche depuis l'écran détail campagne, produisant enfin la donnée que la rule suit.

**Décision de séquençage** : on livre le **link côté joueur (write owner-only)** AVANT la lecture MJ côté UI (qui consommera la donnée). Justification : (a) le link est purement owner-write (le joueur écrit SA membership + SA fiche — 2 docs qu'il possède), donc **aucune rule nouvelle, aucun accès cross-user, aucun deploy requis** ; (b) il est la **pré-condition** de toute UI de lecture MJ (sans fiche liée, le MJ n'a rien à lire) ; (c) il est testable de bout en bout contre l'émulateur sans dépendre du deploy en attente d'Adrien. Le picker est exactement l'UI pour laquelle le service 4A.1 a été écrit (« Appelé par l'UI campaign-detail … à venir »).

**Note sur le commentaire d'écran 4.0.5** : `campaign-detail-screen.tsx` portait « le picker arrive avec le flux Wizard-in-campaign (4C) ». Le commentaire du service 4A.1 (« appelé par l'UI campaign-detail ») a priorité — le picker self-service joueur vit ici dès 4A.2. Un **second** point d'entrée (wizard-in-campaign) pourra s'ajouter en 4C sans conflit. Commentaire d'écran corrigé dans ce commit.

**Livré** :
- `link-character-modal.tsx` : picker présentationnel (radiogroup des fiches du joueur + option « Aucun personnage » pour délier). Le bouton confirme reste désactivé tant que le choix == lien courant (pas de write inutile). Monté conditionnellement par le parent → `useState(currentCharacterId)` se réinitialise à chaque ouverture sans `useEffect` de synchro.
- `my-character-link.tsx` : section « Mon personnage » de l'écran détail. Détient l'unique abonnement `useCharactersList`, résout le nom de la fiche liée pour l'afficher, ouvre la modale. **Visible uniquement pour le joueur** (user avec doc `members/{uid}`) — un MJ pur (gmIds seul) ne la voit pas : il LIT les fiches des joueurs (4A.1), il ne lie pas la sienne.
- `campaign-detail-screen.tsx` : `myMembership` dérivé de `members` ; section rendue entre invite et roster pour les membres.
- i18n FR+EN : namespaces `campaigns.detail.myCharacter.*` + `campaigns.linkCharacter.*`.
- Tests : `link-character-modal.test.tsx` (7 — liste, identité des args du service `(cid,uid,charId)`/`null`, dirty-gate, empty, loading, error) ; `my-character-link.test.tsx` (5 — états none/lié/loading/introuvable + ouverture modale) ; `campaign-detail-screen.test.tsx` étendu (+3 — section visible joueur, nom de fiche liée, masquée MJ pur). e2e `campaigns-link-character.spec.ts` contre l'émulateur (join réel → section pour membre réel → picker empty-state + invariant viewport).

**Périmètre non couvert (volontaire)** : (a) l'UI de **lecture MJ** (le MJ ouvre la fiche d'un joueur lié) reste à câbler — 4A.2 produit la donnée, un jalon suivant la consomme ; (b) le **nettoyage de `homeCampaignId` au délink** reste déféré (inerte, cf. 4A.1) ; (c) la **création de personnage in-campaign** (wizard-in-campaign, 4C) ; (d) pas de write e2e du link via navigateur (couvert en couches : unit args-identité + service batch + rules-émulateur 4A.1) — le full-wizard inline serait brittle, non retenu.
