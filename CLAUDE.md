...
> **Read this in full at the start of every session.** This is the master rulebook. The plans in `docs/plans/` are the work queue. The docs in `docs/` are the architectural reference. The prototype at `docs/prototype/grimwar.html` is the visual reference.
...
Plans in `docs/plans/` are numbered, sequential, sprinted (`S1` table-ready MVP, `S2` campaigns, `S3` DM tools + journal, `S4` map, `S5` polish + launch). See `docs/ROADMAP.md`.
...
| **Sources SRD CC légitimes (LOCKED)** | **PDF officiels `FR_SRD_CC_v5.2.1.pdf` + `SRD_CC_v5.2.1.pdf` dans `content-sources/pdfs/` (source de VÉRIFICATION ultime, en cas de doute sur le formatage ou d'ambiguïté sur une entrée) + leurs extractions texte `content-sources/extracted/raw/{SRD,FR_SRD}_CC_v5.2.1.txt` (source PRIMAIRE des scripts de bootstrap, citée par l'audit D.3).** Toute autre source de contenu D&D (AideDD, PHB copyrighté, livres BBE, etc.) reste interdite par la règle actée 2026-05-16 (« Politique de contenu (LOCKED) »). Acté 2026-05-20 (les PDF SRD binaires confirmés présents sur disque). |
| **`pnpm content:build` (INTERDIT LEVÉ — 13.10b)** | **`pnpm content:build` est de nouveau exécutable.** L'interdit (acté plan 13.7, requalifié 13.10/3 pour cause D17) est **levé le 2026-05-20 au commit `ff2b6f3`** : D9 (sorts AideDD-sourced) résolu plan 13.10, D17 (orchestrateur) résolu plan 13.10b. `build-public-content.ts` est désormais un **orchestrateur idempotent et SRD-only** : il ré-applique les producteurs réels (extracteurs SRD dédiés en sous-process + re-validation Zod de la base SRD curée), n'a **pas** ré-introduit AideDD, et satisfait le critère dur `pnpm content:build && git diff --quiet public/data` → exit 0 (vérifié par `pnpm content:check`). **Caveat de portée (finding B, gravé) :** `content:build` garantit la **REPRODUCTIBILITÉ DE L'ÉTAT LIVE**, **pas la RECONSTRUCTION DEPUIS LES SOURCES BRUTES**. Les types `classes`/`ancestries`/`items` n'ont pas de régénérateur from-scratch — leur base curée vit dans `public/data` et est mise à jour par **enrichissement**, pas par rebuild. Un refresh from-scratch nécessiterait de remettre `content-sources/extracted/srd` à jour contre le schéma courant — hors scope D17, à porter par un plan dédié si jamais nécessaire. **Pass-through (reformulation D17 #2) :** `magic-items.json` (251 items) est **GRANDFATHERED** d'origine AideDD pré-politique LOCKED ; `monsters.json` idem. Leur SRD-sourcing est un plan dédié à venir. `content:build` ne touche **pas** ces bundles (préservés byte-identique en pass-through) — la politique de contenu LOCKED « le build ne lit que du SRD » reste inviolée. Acté 2026-05-17, requalifié 2026-05-20, **levé 2026-05-20** (`ff2b6f3`). |
...
| Visual fidelity | Faithful to `docs/prototype/grimwar.html` (mobile-first), small refinements OK |
| **CI (LOCKED — 13.13)** | **`.github/workflows/ci.yml` : 4 jobs parallèles bloquants** (`static` = typecheck+lint · `unit` = `test:fast` · `matrix` = `test:matrix`, le garde-fou de 13.12 · `emulator` = `test:rules` puis `test:e2e`, Java + firebase-tools + chromium). Triggers `push: main` + `pull_request`. Node 22 (`engines.node` exclut v26), pnpm via corepack (`pnpm@9.12.3`). `pnpm test` complet **n'est pas exécutable en CI** (il embarque `firestore-rules.test.ts` qui se skippe sans émulateur → couverture rules perdue) — d'où le split `fast`+`matrix`+`rules`, qui couvre tout le périmètre Vitest une seule fois. 5 vars `VITE_FIREBASE_*` factices en env clair (valeurs de présence, zéro secret — l'app tourne contre l'émulateur `demo-grimwar`). Acté 2026-05-22. |
| **Flow de développement (LOCKED — 13.13, Voie A)** | **Direct-push sur `main` autorisé par défaut. PROTECTION DOUBLE-COUCHE sur les paths critiques `public/data/**`, `scripts/data/srd-*.ts`, `.github/workflows/**`.** GitHub ne supporte PAS de branch protection conditionnée au path (« PR exigée seulement si paths X » n'existe pas en natif — vérifié 2026-05-22, contre l'hypothèse initiale fausse). Mécanisme réel à 2 étages : **(a) PRÉVENTION LOCALE** — hook git `.githooks/pre-push` (auto-installé sans husky via `core.hooksPath`, posé par le script `prepare` de `package.json`) refuse un push direct sur `main` qui touche ces paths (message explicite : utiliser une PR, ou contourner consciemment avec `--no-verify`). **(b) DÉTECTION CI** — job `protected-paths-guard` (`ci.yml`, `if: push` only) échoue si un commit **direct non-merge** (`git rev-list --first-parent --no-merges`) sur `main` touche ces paths ; filet pour les `--no-verify` et les pushes depuis une autre machine. Sémantique partagée par les 2 couches via `scripts/ci/protected-paths.sh` (source unique des patterns). **Un merge de PR n'est jamais bloqué** (merge commit exclu) → la PR est la voie légitime des paths protégés. Justification : ces paths sont les bundles SRD durcis par 13.7-13.10b + la pipeline CI elle-même ; mécanisme à 2 étages = sweet spot solo (zéro friction sur paths normaux, vraie prévention sur paths critiques). Les 4 required status checks GitHub (`static`/`unit`/`matrix`/`emulator`) ne mordent qu'en flow PR ; sous direct-push ils restent informatifs (badge). Acté 2026-05-22. |
| **DM omni-edit (LOCKED — plan 26, Voie B)** | **L'autorité d'écriture MJ sur la fiche d'un joueur lié est « rules-only » — AUCUNE Cloud Function, AUCUN plan Blaze.** `firestore.rules > allow update` autorise le MJ via la même autorité *live* que la lecture A2 (`gmCanReadLinkedCharacter` : MJ ∈ `gmIds` de la `homeCampaignId`, joueur encore membre, fiche encore liée) + `dmOmniEditLockedFieldsUnchanged()` qui impose l'immuabilité des champs réservés au propriétaire (`name`, `personality`, `homeCampaignId`). Côté client, `useUpdateCharacter` lit `ownerUid`/`isDMEdit`/`lockedFields` du `PermissionProvider` (posé par `CampaignMemberSheetScreen`), route l'écriture vers `users/{ownerUid}/...` et journalise un événement d'audit `dm-edit` (visibilité `all`). **⚠ SUPERSEDE la décision plan 16 « omni-edit via Cloud Function (Admin SDK) post-S2 »** : le pointeur unique `homeCampaignId` (compromis A2 « une campagne d'attache à la fois ») a levé l'obstacle cité par plan 16 (« arrays cross-collection non vérifiables côté rules »). Conséquence transverse : **tant que ce modèle tient, aucune fonctionnalité MJ-authority n'introduit de backend Functions** (l'app reste client + Firestore rules). Acté 2026-06-25 sur « avance à fond » d'Adrien (recommandation Voie B explicite). |
...
- **Arbitrage EN↔FR sur corruption d'une extraction `.txt` SRD.** Acté 2026-05-20 (plan 13.10 commit 2, validé par Adrien après la reconstruction manuelle du scramble Animate Objects / Antilife Shell / Antipathy et la résolution du conflit d'upcast de Domination de personne — vérifiée page 138 du PDF FR via Domination de bête/de monstre intactes). Quand l'extraction texte SRD (`content-sources/extracted/raw/{SRD,FR_SRD}_CC_v5.2.1.txt`) est corrompue d'un côté (interleaving, troncature, fragment parasite) et que l'EN et le FR divergent : **(a)** l'**EN tranche pour la MÉCANIQUE** (chiffres, niveaux d'upcast, dés, jets de sauvegarde, durées) — c'est la source non corrompue de référence ; **(b)** le **FR tranche pour la FORMULATION** (traduction officielle du phrasé), repris des fragments `.txt` FR non corrompus ou du PDF FR ; **(c)** le commit **cite explicitement** la justification de l'arbitrage avec la référence à la source (ligne `.txt` EN ou page du PDF) ; **(d)** quand un sort « voisin » intact dans la même chaîne sémantique existe (ex. Domination de bête pour Domination de personne), il sert de **vérification croisée** pour la progression mécanique. Cet arbitrage ne s'autorise qu'en dernier recours, sur corruption avérée des deux extractions — pas comme raccourci pour trancher un doute terminologique (qui relève de la règle « Source d'autorité terminologique FR » ci-dessus).
- **Livraison visuelle — dossier `uat-review/` unique par commit UI, captures TOUJOURS en pleine page.** À chaque commit qui touche l'UI, Claude rassemble dans `uat-review/` à la racine du projet (gitignored) UNIQUEMENT les captures-clés que l'utilisateur doit juger pour CE commit — pas la galerie e2e brute, pas les 20 PNG du run, seulement les écrans qui montrent ce qui a changé ou ce qui reste à valider sensoriellement. Nommage : `NN-description-courte.png` (préfixe numéroté à 2 chiffres + description en kebab-case français), dans l'ordre exact où l'utilisateur doit les regarder. **Toutes les captures sont en pleine page** (`fullPage: true` côté Playwright) — toute la hauteur scrollable du document, de haut en bas. Une capture viewport-only tronque le contenu hors écran et ne permet PAS une vraie validation. Exception : si un panneau interne a son propre `overflow` indépendant du scroll du document (rare), `fullPage` ne le capturera pas — dans ce cas Claude doit le signaler explicitement à l'utilisateur dans le rapport et compléter par un screenshot ciblé sur l'élément. Le rapport de livraison DOIT contenir : (a) le chemin absolu du dossier, (b) la liste ordonnée des fichiers avec UNE phrase par fichier disant ce qu'il faut vérifier dessus (« vérifier que les 2 sections sont visuellement distinctes », « vérifier le contraste du titre doré », etc.). Après validation (ou liste de corrections), `uat-review/` est vidé avant le commit suivant — au début de chaque commit UI, Claude commence par `rm -rf uat-review && mkdir -p uat-review` (et **non** `rm -rf uat-review/*`, qui échoue en `no matches found` sous zsh quand le dossier est déjà vide, et coupe alors une chaîne `&&`) puis re-remplit avec les captures pertinentes pour le nouveau commit. Pas d'accumulation cross-commit. Acté 2026-05-19 suite au commit 4c du plan 13.9 — les screenshots attachés au rapport Playwright HTML étaient éparpillés test par test et pénibles à parcourir ; le dossier dédié réduit le coût de l'UAT visuel à une seule galerie ordonnée. Mise à jour 2026-05-19 (même jour) suite à l'UAT 4c : les premières captures viewport-only ne montraient qu'un fragment de la fiche — `fullPage: true` devient obligatoire. Mise à jour 2026-05-20 (modales) : pour toute capture d'une **modale ouverte**, ajouter une **deuxième capture `viewport: true`** en plus de la pleine page — `fullPage` reprojette la modale dans le flux du document et masque le ressenti d'overlay (backdrop, ancrage `items-end` mobile, opacité, stacking) ; la viewport-only restitue l'overlay. Le contenu textuel exhaustif se lit sur la pleine page, le ressenti visuel sur la viewport. Le helper `tests/e2e/helpers/screenshot.ts` accepte une option `{ viewport: true }` pour la seconde capture. Mise à jour 2026-05-20 (overflow interne) suite à l'UAT 13.10/3 — capture 00 tronquée : **`fullPage: true` ne franchit PAS un `overflow-y-auto` posé sur un conteneur de hauteur fixe interne au document** (Playwright étend le viewport à la hauteur du `<html>`, pas à la hauteur de contenu d'un panneau scrollé indépendamment). Avant de capturer, vérifier le DOM : si la zone à capturer scrolle DANS un conteneur de hauteur figée (`max-h-*` / `h-[…]` + `overflow-y-auto`) plutôt qu'au niveau du document, `fullPage` rendra une capture tronquée *silencieusement*. Stratégie dans ce cas : (a) scroll programmatique + captures multiples concaténées, OU (b) agrandissement temporaire du viewport (`page.setViewportSize`) pour que tout tienne sans scroll interne, OU (c) override CSS de la hauteur du conteneur via Playwright. Choisir la plus propre. NB : pour la fiche GrimWar, la liste de sorts scrolle au niveau du document (`<main className="min-h-screen">`, aucun overflow interne sur le chemin `spell-list.tsx`) — `fullPage` y suffit ; la troncature 00 était un simple oubli de `fullPage`, pas un piège d'overflow. **Mise à jour 2026-08-04 (recuration obligatoire après tout run e2e complet) :** `uat-review/` est **UN SEUL dossier PLAT contenant UNIQUEMENT les captures du lot courant** — jamais de sous-dossier, jamais de galerie d'un autre lot. L'accumulation est **structurelle** et non accidentelle : chaque spec UAT écrit dans `uat-review/`, donc `pnpm test:e2e` complet y redépose les galeries de TOUTES les specs, même quand le lot n'en touche qu'une. Conséquence : après chaque run e2e, **recurer explicitement** (supprimer tout ce qui n'appartient pas au lot livré, y compris les sous-dossiers hérités et `.DS_Store`) et **renuméroter `01-…` en séquence** dans l'ordre de revue — un préfixe hérité (`30-…`) n'a de sens que si le lot 1-29 cohabite, ce qui ne doit jamais arriver. Cette consigne **remplace** l'ancienne « garder les galeries non validées jusqu'à validation » : si un lot antérieur n'a pas été validé, le signaler en une ligne dans le rapport et proposer de le régénérer, pas le laisser encombrer le dossier. Vérifier enfin que **les captures sont postérieures au dernier commit du lot** — une capture antérieure à un correctif montre l'état bogué et invaliderait l'UAT.
...
- `docs/plans/00-overview.md` — full plan index
