...
| **Interdit `pnpm content:build` (REQUALIFIÉ)** | **NE PAS exécuter `pnpm content:build`.** L'interdit (acté plan 13.7) reste en force, mais pour la **bonne raison** désormais. **Cause initiale (D9 — sorts AideDD-sourced) résolue plan 13.10.** **NOUVELLE CAUSE découverte 2026-05-20 commit 13.10/3 :** `build-public-content.ts` est obsolète — il lit un `SRD_DIR` figé et ignore les extracteurs dédiés `extract-srd-*.ts`, ce qui vide classes/ancestries/feats/invocations de leurs bundles live et régresse l'index. Refonte requise comme **orchestrateur** (lance les extracteurs SRD dédiés + merge AideDD seulement pour `magic-items`/`monsters` tant que ces types ne sont pas SRD-sourced). Owner = **plan dédié post-13.10** ; dette tracée `plans/DEBT.md > D17`. Acté 2026-05-17, requalifié 2026-05-20. |
