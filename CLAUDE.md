...
| **Flow de développement (LOCKED — 13.13)** | **Direct-push sur `main` autorisé par défaut. Paths protégés exigeant une PR (branch protection + required checks `static`/`unit`/`matrix`/`emulator`) : `public/data/**`, `scripts/data/srd-*.ts`, `.github/workflows/**`.** Justification : ces paths sont les bundles SRD durcis par les plans 13.7-13.10b + la pipeline CI elle-même — une régression silencieuse y coûte cher et la CI seule en post-hoc serait trop tard. Pour les autres paths, le coût d'une PR systématique en solo dépasse le bénéfice. La branch protection est un réglage de repo GitHub (UI/Settings), pas gravable dans un commit. Acté 2026-05-22. |
